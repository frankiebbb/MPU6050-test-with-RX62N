...
#if defined MOTION_DRIVER_TARGET_RENESAS
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
		...
		i2c_read(st.hw->addr, st.reg->int_enable, 1, &tmp1);
		delay_ms(100);
		
		
		uint8_t lcd_buffer[13] = {0};
		sprintf((char *) lcd_buffer, "   %d   ", tmp1);   
...
int mpu_init(void/*struct int_param_s *int_param*/)
{
    ...
    /* Wake up chip. */
    data[0] = 0x00;
	result = i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data);
	if (result)
        return -1;
...
int mpu_run_self_test(long *gyro, long *accel)
{
	...
	/*unsigned char lcd_buffer[13] = {0};
    sprintf((char *) lcd_buffer, "   %d   " , gyro_result);   
